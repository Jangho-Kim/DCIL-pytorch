...
for d in cifar10 cifar100; do
  for p in 0.9 0.95; do
    for s in 6 7 8; do
      python run_dcil.py ${d} --datapath ${datapath} -a resnet --layers 56 -C -g 0 -P --prune-type unstructured \
      ...
      --target_epoch 225 --save dcil_sparsity${p}_seed${s}.pth  | tee log/dcil_res56_${d}_sparsity${p}_seed${s}.txt
    done
  done
done
